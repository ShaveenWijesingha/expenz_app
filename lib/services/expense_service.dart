...
class ExpenseService {
  ...
  //delete the expense from the shared pref from the id
  Future<void> deleteExpense(int id, BuildContext context) async {
    try {
      ...
      //remove the expense with the given id from the list
      existingExpensesObjects.removeWhere((expense) => expense.id == id);
      //convert the list of expense back to a list of strings
  ...
  Future<void> deleteAllExpenses(BuildContext context)async{
    try{
      ...
      //show message
      if(context.mounted){
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("All Expenses deleted"),duration: Duration(seconds: 2),));
      }
    }catch(error){
      //show message
      if(context.mounted){
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("error"),duration: Duration(seconds: 2),));
      }
    }
